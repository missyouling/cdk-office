...
func TestAppPermissionService_CreateAppPermission(t *testing.T) {
	// Setup
	...
	tests := []struct {
		name          string
		request       *service.CreateAppPermissionRequest
		expectError   bool
		errorMessage  string
	}{
		{
			name: "Valid app permission creation",
			request: &service.CreateAppPermissionRequest{
				AppID:       "app-001",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Execute
			permission, err := appPermissionService.CreateAppPermission(context.Background(), tt.request)
			...
			if tt.expectError {
				...
				assert.Equal(t, tt.request.Permission, permission.Permission)
...
func TestAppPermissionService_UpdateAppPermission(t *testing.T) {
	// Setup
	db := testutils.SetupTestDB()
	defer db.Migrator().DropTable(&domain.AppPermission{}, &domain.Application{})
	...
	tests := []struct {
		name          string
		permissionID  string
		request       *service.UpdateAppPermissionRequest
		expectError   bool
		errorMessage  string
	}{
		{
			name:         "Valid app permission update",
		...
		{
			name:         "Update non-existent app permission",
			permissionID: "non-existent-id",
			request: &service.UpdateAppPermissionRequest{
				Name: "Updated Name",
			},
			expectError:  true,
			errorMessage: "application permission not found",
		},
		{
			name:         "Invalid permission",
			permissionID: "perm-001",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Execute
			err := appPermissionService.UpdateAppPermission(context.Background(), tt.permissionID, tt.request)
			...
			if tt.expectError {
				...
				// Verify the update
				updatedPermission, getErr := appPermissionService.GetAppPermission(context.Background(), tt.permissionID)
				...
				if tt.request.Permission != "" {
					assert.Equal(t, tt.request.Permission, updatedPermission.Permission)
				}
...
func TestAppPermissionService_DeleteAppPermission(t *testing.T) {
	// Setup
	db := testutils.SetupTestDB()
	...
	tests := []struct {
		...
		{
			name:         "Delete non-existent app permission",
			permissionID: "non-existent-id",
			expectError:  true,
			errorMessage: "application permission not found",
		},
		{
			name:         "Valid app permission deletion",
			permissionID: "perm-001",
			expectError:  false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Execute
			err := appPermissionService.DeleteAppPermission(context.Background(), tt.permissionID)
			...
			if tt.expectError {
				...
				// Verify app permission is deleted
				_, getErr := appPermissionService.GetAppPermission(context.Background(), tt.permissionID)
				assert.Error(t, getErr)
				assert.Equal(t, "application permission not found", getErr.Error())
...
func TestAppPermissionService_ListAppPermissions(t *testing.T) {
	// Setup
	db := testutils.SetupTestDB()
	defer db.Migrator().DropTable(&domain.AppPermission{}, &domain.Application{})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Execute
			result, total, err := appPermissionService.ListAppPermissions(context.Background(), tt.appID, tt.page, tt.size)
...
func TestAppPermissionService_GetAppPermission(t *testing.T) {
	// Setup
	db := testutils.SetupTestDB()
	defer db.Migrator().DropTable(&domain.AppPermission{}, &domain.Application{})
	...
	tests := []struct {
		...
		{
			name:         "Get non-existent app permission",
			permissionID: "non-existent-id",
			expectError:  true,
			errorMessage: "application permission not found",
		},
		{
			name:         "Get existing app permission",
			permissionID: "perm-001",
			expectError:  false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Execute
			result, err := appPermissionService.GetAppPermission(context.Background(), tt.permissionID)
...
func TestAppPermissionService_AssignPermissionToUser(t *testing.T) {
	// Setup
	db := testutils.SetupTestDB()
	defer db.Migrator().DropTable(&domain.AppPermission{}, &domain.AppUserPermission{}, &domain.Application{})
	...
	tests := []struct {
		name          string
		request       *service.AssignPermissionToUserRequest
		expectError   bool
		errorMessage  string
	}{
		{
		...
		},
		{
			...
			expectError:  true,
			errorMessage: "application permission does not belong to the specified application",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Execute
			err := appPermissionService.AssignPermissionToUser(context.Background(), tt.request)
...
func TestAppPermissionService_RevokePermissionFromUser(t *testing.T) {
	// Setup
	db := testutils.SetupTestDB()
	defer db.Migrator().DropTable(&domain.AppPermission{}, &domain.AppUserPermission{}, &domain.Application{})
	...
	tests := []struct {
		name          string
		request       *service.RevokePermissionFromUserRequest
		expectError   bool
		errorMessage  string
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Execute
			err := appPermissionService.RevokePermissionFromUser(context.Background(), tt.request)
...
func TestAppPermissionService_ListUserPermissions(t *testing.T) {
	// Setup
	db := testutils.SetupTestDB()
	defer db.Migrator().DropTable(&domain.AppPermission{}, &domain.AppUserPermission{}, &domain.Application{})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Execute
			result, err := appPermissionService.ListUserPermissions(context.Background(), tt.appID, tt.userID)
...
func TestAppPermissionService_CheckUserPermission(t *testing.T) {
	// Setup
	db := testutils.SetupTestDB()
	defer db.Migrator().DropTable(&domain.AppPermission{}, &domain.AppUserPermission{}, &domain.Application{})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Execute
			result, err := appPermissionService.CheckUserPermission(context.Background(), tt.appID, tt.userID, tt.permission)
